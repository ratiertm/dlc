...
### Active (v2.0 — gstack 패턴 차용)

- [ ] **lifecycle-config** — `get/set` CLI 스타일 설정 관리 (mode, skip_stages, proactive, auto_skip)
- [ ] **Stage-internal iteration** — DO 안에서 step별 mini-verify 루프 (QA→Fix→Verify 패턴)
- [ ] **lifecycle-upgrade** — 스키마 마이그레이션 + 롤백 + 변경사항 요약
- [ ] **Completeness 점수** — 각 스테이지 N/10 점수, 의사결정 시 Completeness 비교
- [ ] **Proactive suggestions + opt-out** — 스테이지 전환 시 관련 스킬 추천, proactive: false로 끄기
- [ ] **Session tracking** — 세션별 context 파일 (.lifecycle/sessions/)
- [ ] **Migration markers** — .lifecycle/.v2-migrated 등 안전한 업그레이드 표시
- [ ] **Before/after snapshot diff** — 스펙 baseline 대비 구현 delta report
- [ ] **Analytics** — 스테이지별 병목 추적, 재작업 이벤트, time-per-stage
- [ ] **gstack 스킬 연동** — review, qa, investigate, careful 등 적절한 시점에 추천
