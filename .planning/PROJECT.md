...
- [ ] **[핵심] User Interaction Prototype** — PLAN 단계에서 클릭 가능한 HTML+JS 목업을 생성하여 사용자와 합의. 이것이 없으면 "완성했다고 하는데 버튼이 없다/안 된다" 문제가 100% 반복됨. PLAN의 핵심 산출물이지 별도 Phase가 아님
- [ ] **[핵심] End-to-end 기능 명세** — 각 기능을 화면→연결→처리→응답→에러 5단계로 명세. PLAN에서 작성, DO에서 구현 기준, TEST에서 검증 기준. 파이프라인 전체를 관통하는 핵심 포맷
